...
export default function ServiceContract() {
  ...
  const routeChangeAdd = () => {
    let path = `/vendorinfo/AddServiceContract`;
    navigate(path);
  };
  ...
  const accept = () => {
    ...
    api
      .post(`ServiceContract/DeleteVendorContractInformation`, collectData)
      .then((response) => {
  ...
  const fetchServiceContractData = async () => {
    try {
      const collectData = {
        "contractId": -1
      };
      const response = await api.post(
        `ServiceContract/GetVendorContractInformation`,
        collectData
      );
      const data = response.data.data;
