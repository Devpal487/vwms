...
const LightInstallationEdit = () => {
  ...
  const getInstallationById = async (id: any) => {
    const result = await api.post(`LightInstallation/GetLightInstallation`, {
      lightInstId: id,
      zoneId: -1,
      wordId: -1,
    });
    const transData = result?.data?.data[0][`lightInstalChild`];
    console.log('transData',transData)

    formik.setFieldValue('apprImg', result?.data?.data[0][`apprImg`])

    let arr: any = [];
    for (let i = 0; i < transData.length; i++) {
      arr.push({
        lightInstallationId: transData[i]["lightInstallationId"],
        lightInstId: transData[i]["lightInstId"],
        itemId: transData[i]["itemId"],
        lightNo: transData[i]["lightNo"],
        walt: transData[i]["walt"],
  ...
  const formik = useFormik({
    initialValues: {
      lightInstId: location.state.id,
      zoneId: location.state.zoneId,
      wordId: location.state.wordId,
      poleNo: location.state.poleNo,
      instDate: moment(location.state.instDate).format("YYYY-MM-DD"),
      instBy: location.state.instBy,
      apprBy: location.state.apprBy,
      apprImg: location.state.apprImg,
      apprDate: moment(location.state.apprDate).format("YYYY-MM-DD"),
            ...
            <Grid item xs={12} container spacing={2}>
              ...
              <Grid item lg={6} xs={12}>
                <Autocomplete
                  ...
                  value={
                    taxOption.find(
                      (option: any) => option.value === formik.values.wordId
                    ) || null
                  }
                  fullWidth
                  size="small"
                        ...
                        <tr key={item.id} style={{ border: "1px solid black" }}>
                          <td style={{ width: "300px" }}>
                            <Autocomplete
                              ...
                              value={
                                contentOptions.find(
                                  (option: any) => option.value === item?.itemId
                                ) || null
                              }
                              size="small"
                              onChange={(event, newValue: any) => {
...
export default LightInstallationEdit;
