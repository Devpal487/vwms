...
const EditIndentApproval = (props: Props) => {
    ...
    useEffect(() => {
        ...
        GetitemData();
        GetUnitData();
        getIndentStoreBYId(location.state.id);
    }, []);

    const getIndentStoreBYId = async(id:any)=>{
        const collectData = {
            indentID: id,
        };
        const response = await api.post(`indentParent/GetindentParent`, collectData);
        const data = response.data.data[0]['indentinv'];
    ...
    const formik = useFormik({
        initialValues: {
            ...
            reqBefDate: location.state.reqBefDate ? dayjs(location.state.reqBefDate).format("YYYY-MM-DD") : "",
            deptID: location.state.deptID,
            emp_name: location.state.emp_name,
            remark: location.state.remark || "",
            instID: location.state.instID || 0,
            sessionID: location.state.sessionID || 0,
            approve: location.state.approve || "",
            user_id: location.state.user_id || 0,
            divisionId: location.state.divisionId || 0,
            forward_Status: location.state.forward_Status || "",
            forward_By: location.state.forward_By || 0,
            forward_On: defaultValues, 
            storeId: location.state.storeId || 0,
            l3_Status: location.state.l3_Status || "",
            l3_By: location.state.l3_By || 0,
            l3_On: defaultValues,  
            mode: location.state.mode || "",
            estimateNo: location.state.estimateNo || "",
            workType: location.state.workType || "",
            proposalNo: location.state.proposalNo || "",
            patCodeNo: location.state.patCodeNo || "",
            toStoreId: location.state.toStoreId || 0,
            toDivisionId: location.state.toDivisionId || 0,
            is_PMN: location.state.is_PMN || "",
            stage: location.state.stage || "",
            indentinv: location.state.indentinv || [], 
        },
                        ...
                        <Grid item xs={12} container spacing={2}>
                            ...
                            <Grid item xs={12} sm={4} lg={4}>
                                <Autocomplete
                                    ...
                                    // } }
                                    value={
                                        indentOptions.find((opt) => (opt.value) === (formik.values.indentID)) || null
                                    }
                            ...
                            <Grid item lg={4} xs={12}>
                                <TextField
                                    id="reqBefDate"
                                    name="reqBefDate"
                                    label={<CustomLabel text={t("text.reqBefDate")} required={false} />}
                                    value={formik.values.reqBefDate}
                                    placeholder={t("text.reqBefDate")}
                                    size="small"
                                    fullWidth
                                    type="date"
                                    onChange={formik.handleChange}
                            ...
                            <Grid item lg={4} xs={12}>
                                <TextField
                                    id="emp_name"
                                    name="emp_name"
                                    label={<CustomLabel text={t("text.emp_name")} required={false} />}
...
export default EditIndentApproval;