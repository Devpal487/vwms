 ...
 // console.log("items", items);
  ...
  const MapData = () => {
    ...
    const arr = validItems.map((item: any, index: any) => ({
      ...
      icon: "https://cdn-icons-png.flaticon.com/512/209/209403.png",
    }));
    setVehicles(arr);
  ...
  useEffect(() => {
    getInstallationById(location.state.id);
  ...
  const getInstallationById = async (id: any) => {
    const result = await api.post(`PoleInstallation/GetPoleInstallation`, {
      poleInstId: id,
      zoneId: -1,
      wardId: -1,
    });
    const transData = result?.data?.data[0]["poleInstalChild"];

    let arr: any = [];
    for (let i = 0; i < transData.length; i++) {
      arr.push({
        poleInstChildId: transData[i]["poleInstChildId"],
        poleInstId: transData[i]["poleInstId"],
        itemid: transData[i]["itemid"],
        poleno: transData[i]["poleno"],
        installOfLight: transData[i]["installOfLight"],
        latitute: transData[i]["latitute"],
        longitute: transData[i]["longitute"],
        empId: transData[i]["empId"],
        depth: transData[i]["depth"],
        apprby: transData[i]["apprby"],
        poleimg: transData[i]["poleimg"],
        apprdate: moment(transData[i]["apprdate"]).format("YYYY-MM-DD"),
        islightinst: transData[i]["islightinst"],
        itemName: transData[i]["itemName"],
        empName: transData[i]["empName"],
      });
    }
    setItems(arr);
  };
  ...
  const formik = useFormik({
    initialValues: {
      poleInstId: location.state.id,
      poleInstdate: moment(location.state.poleInstdate).format("YYYY-MM-DD"),
      isuueidentid: location.state.isuueidentid,
                      ...
                      {currentMap && (
                        ...
                        >
                          ...
                          <GoogleMap
                            ...
                            center={center}
                            zoom={40} // Default zoom, will be adjusted by `fitBounds`
...
export default PoleInstallationEdit;
