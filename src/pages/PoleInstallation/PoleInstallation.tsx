...
export default function PoleInstallation() {
  ...
  const routeChangeEdit = (row: any) => {
    ...
    let path = `/installation/PoleInstallationEdit`;
    navigate(path, {
      state: row,
    });
  };
  ...
  const routeChangeAdd = () => {
    let path = `/installation/PoleInstallationAdd`;
    navigate(path);
  };
  ...
  const accept = () => {
    const collectData = {
      poleInstId: delete_id,
      // user_ID: 0,
    };
    console.log("collectData " + JSON.stringify(collectData));
    api
      .post(`PoleInstallation/DeletePoleInstallation`, collectData)
      .then((response: any) => {
        if (response.data.isSuccess) {
          toast.success(response.data.mesg);
        } else {
  ...
  const fetchZonesData = async () => {
    try {
      ...
      const response = await api.post(`PoleInstallation/GetPoleInstallation`, collectData);
      ...
      if (data.length > 0) {
        const columns: GridColDef[] = [
          ...
          {
            field: "poleInstdate",
            headerName: t("text.PoleInstallationDate"),
            flex: 1,
            headerClassName: "MuiDataGrid-colCell",
          },
          {
